...
import { setEditTaskVisibility, setTaskInfoState } from '../actions/modal-visibility';
...
class TaskTable extends Component {
			...
			switch (this.props.filteredTasks.type) {
				...
				case 'title':
					return this.props.fetchedTasks.filter(item => {
						return (
							item.name.toLowerCase().indexOf(this.props.filteredTasks.text.toLowerCase()) > -1
						);
					});
				case 'client':
					return this.props.fetchedTasks.filter(item => {
	...
	render() {
		const tableData = this.convertToTableData(this.checkFilterType());
			...
			<div className="task_table_container">
				<Table bordered={true} pagination={false} dataSource={tableData} expandRowByClick={true}>
						...
						filterDropdown={
							<div className="custom_filter_dropdown">
								<Input
									...
									onPressEnter={() => this.props.loadTableFilter(this.state.searchText, 'title')}
								/>
								<Button
									className="search_btn"
									type="primary"
									onClick={() => this.props.loadTableFilter(this.state.searchText, 'title')}
									icon="search"
								>
									Search
								</Button>
						...
						filterDropdown={
							<div className="custom_filter_dropdown">
								<Input
									...
									onPressEnter={() =>
										this.props.loadTableFilter(this.state.clientSearchText, 'client')
									}
								/>
								<Button
									className="search_btn"
									type="primary"
									onClick={() => this.props.loadTableFilter(this.state.clientSearchText, 'client')}
									icon="search"
								>
									Search
								</Button>
					...
					<Column
						...
						render={(text, record) => (
							<span>
								<a onClick={() => this.handleInfoClick(record.key)}>More info</a>
