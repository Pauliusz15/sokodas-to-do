...
class EditClient extends Component {
	...
	handleOk() {
		...
		if (
			...
			this.props.editClient(
				...
				this.state.nameInput ? this.state.nameInput : this.props.loadedClient.name,
				this.state.codeInput ? this.state.codeInput : this.props.loadedClient.code,
				this.state.pvmCodeInput ? this.state.pvmCodeInput : this.props.loadedClient.pvm_code,
				this.state.commentInput ? this.state.commentInput : this.props.loadedClient.comment
			);
	...
	render() {
		const suffixName = this.state.nameInput ? (
			<Icon type="close-circle" onClick={() => this.setState({ nameInput: '' })} />
		) : null;
		const suffixCode = this.state.codeInput ? (
			<Icon type="close-circle" onClick={() => this.setState({ codeInput: '' })} />
		) : null;
		const suffixPvmCode = this.state.pvmCodeInput ? (
			<Icon type="close-circle" onClick={() => this.setState({ pvmCodeInput: '' })} />
		) : null;
				...
				>
					<Form layout="vertical">
						...
						>
							<Input
								value={
									this.state.nameInput != null ? this.state.nameInput : this.props.loadedClient.name
								}
						...
						<FormItem
							...
							help={this.state.codeInputError ? 'You must enter a company code' : ''}
							label="Company code"
							required={true}
						>
							<Input
								...
								value={
									this.state.codeInput != null ? this.state.codeInput : this.props.loadedClient.code
								}
