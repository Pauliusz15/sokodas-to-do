...
class EditTask extends Component {
  ...
  showExpired() {
    let prog = this.state.progressInput
      ? this.state.progressInput
      : this.props.loadedTask.progress;
    if (prog !== "Completed") {
      if (
        getCurrentDate() >
        checkDateTime(
  ...
  handleOk() {
    ...
    if (this.state.titleInput !== "") {
      this.props.editTask(
        ...
        this.state.currency
          ? this.state.currency
          : this.props.loadedTask.currency,
        this.state.progressInput
          ? this.state.progressInput
          : this.props.loadedTask.progress,
        this.showExpired()
        ...
        >
          <Form layout="vertical">
            ...
            <FormItem className="inline_block" label="Time to be done">
              <TimePicker
                defaultValue={moment(
                  this.props.loadedTask.date.substr(
                    this.props.loadedTask.date.length - 5
                  ),
                  "HH:mm"
                )}
                format={"HH:mm"}
